...
首页呈现遵循用户 2026-09-23 批准的简洁方向：白色与浅灰、大标题、留白、蓝色主阅读按钮。移除已归档日报、最新一期和月份覆盖统计。首屏直接引导阅读最大报道日期的完整日报；下方保持 7 日、4 周、3 月窗口。每个 daily/weekly/monthly 条目必须提供 `card_title`（最多 24 字符）和 `card_deck`（最多 60 字符）：用具体、克制的短句概括变化，不夸大事实，不照搬品牌广告语。日报卡显示简短导读并链接全文；周月卡默认显示简短导读，完整分析、重点、来源和后续观察可展开。缺期说明保持可见。底层 `title`、`overview`、`items` 和日报全文的深度不因首页简化而删减。
